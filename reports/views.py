...
def _export_queryset_and_title(request):
    ...
    if "-" in month_param:
        try:
            ...
            return qs, f"Bao cao su co thang {month:02d}/{year}", f"bao_cao_{year}_{month:02d}.docx"
    ...
    if year_param:
        try:
            ...
            return qs, f"Bao cao su co nam {year}", f"bao_cao_nam_{year}.docx"
        except ValueError:
            pass

    return MaintenanceRequest.objects.all(), "Bao cao su co ha tang do thi", "bao_cao_su_co.docx"
...
def export_word(request):
    ...
    qs, title, filename = _export_queryset_and_title(request)
    now = timezone.now()

    doc = Document()
    heading = doc.add_heading(title.upper(), level=1)
    heading.alignment = WD_ALIGN_PARAGRAPH.CENTER
    doc.add_paragraph(f"Ngay xuat: {now:%d/%m/%Y %H:%M}")
    ...
    doc.add_heading("Thong ke tong quan", level=2)
    ...
    summary_rows = [
        ...
        ("Cho xu ly", pending),
        ("Ty le hoan thanh", f"{round(completed / total * 100) if total else 0}%"),
        ("Tong chi phi du kien", f"{total_cost:,.0f} VND"),
    ]
    ...
    doc.add_heading("Danh sach su co", level=2)
    table = doc.add_table(rows=1, cols=6)
    table.style = "Table Grid"
    headers = ["STT", "Tieu de", "Loai", "Trang thai", "Nguoi bao", "Ngay tao"]
    for idx, header in enumerate(headers):
        table.rows[0].cells[idx].text = header

    for index, incident in enumerate(qs.select_related("reported_by")[:50], 1):
        row = table.add_row().cells
        row[0].text = str(index)
        row[1].text = incident.title or ""
        row[2].text = str(
            incident.get_incident_type_display()
            if hasattr(incident, "get_incident_type_display")
            else incident.incident_type
            or ""
        )
        row[3].text = str(
            incident.get_status_display() if hasattr(incident, "get_status_display") else incident.status or ""
        )
        row[4].text = incident.reported_by.get_full_name() if incident.reported_by else ""
        row[5].text = incident.created_at.strftime("%d/%m/%Y") if incident.created_at else ""
